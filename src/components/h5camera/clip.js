...
//因为要保存到vue实例中，所以直接传一个$vm当闭包使用了
  /***
   * constructor 构造函数 画盒子
   * @param wpId
   * @param $vm
   */
  constructor(wpId, $vm) {
    this.regional = document.getElementById(wpId);
    this.getImage = document.createElement('canvas'); // 展示图片的canvas
    this.getImage.id = 'image-box';
    this.editBox = document.createElement('canvas');  // 剪裁图片的canvas
  ...
  paintImg(picUrl) {
    ...
    img.onload = function () {
      ...
      let imgScale = img.width / img.height; // 选择的图片的宽高比
      let boxScale = t.regional.offsetWidth / t.regional.offsetHeight; // id是container的盒子的宽高比
      console.log(imgScale,boxScale,t.chooseBoxScale)
      //判断盒子与图片的比列
